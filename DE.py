...
def RandomSearch(model, poly, space):
    dim = len(space)
    size = 1000
    samples = np.zeros((size, dim))
    for i in range(size):
        for j in range(dim):
            samples[i][j] = np.random.uniform(space[j][0], space[j][1])
    samples_poly = poly.fit_transform(samples)
    fitness = model.predict(samples_poly)
    return samples[np.argmin(fitness)]
